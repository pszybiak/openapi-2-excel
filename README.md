...
referenced schema, and `nullable` or `deprecated` counts when either of the two declares it. A
composition of several schemas has no single name, so it stays an `object`.
