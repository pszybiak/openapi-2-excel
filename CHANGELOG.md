...
- `Object type` names the schemas a composition is written as and says how they are to be read,
  `All of (Cat, Dog)`, `One of (Cat, Dog)`, `Any of (Cat, Dog)`, instead of calling it an object.
  The three labels are part of the translation, so the document says it in its own language.
